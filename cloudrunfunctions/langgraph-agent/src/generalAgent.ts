...
async function createAgentDescription(
  ...
  const prompt = `我在用代码构建一个 Agent，需要一段对 Agent 的描述。我给这个 Agent 提供了一系列的工具，请你帮我据此生成一段对 Agent 的描述吧。你需要精准地告诉他他能做什么事，不要输出 Agent 描述之外的内容。工具列表如下： ${tools.map((x) => `工具名称：${x.name}，工具描述：${x.description}`).join("\n")}`;
